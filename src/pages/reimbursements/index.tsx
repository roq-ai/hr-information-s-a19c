...
import { useRoqClient, useReimbursementFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ReimbursementInterface, unknown>;

interface ReimbursementListPageProps {
...
export function ReimbursementListPage(props: ReimbursementListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useReimbursementFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.reimbursement.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ReimbursementInterface) => {
    if (hasAccess('reimbursement', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/reimbursements/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'status', header: 'Status', accessorKey: 'status' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.request_date ? format(record?.request_date, 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/reimbursements/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('reimbursement', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/reimbursements/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('reimbursement', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="reimbursement" />
        </Flex>

        {hasAccess('reimbursement', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/reimbursements/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
