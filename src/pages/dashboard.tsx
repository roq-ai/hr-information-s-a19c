...
import { useRoqPlatformUserProfiles, useReimbursementCount, useVacationCount, useSickLeaveCount } from 'lib/roq';

import { SickLeaveListPage as ShowCaseList } from 'pages/sick-leaves';

function DashboardPage() {
  const { session } = useSession();

  const reimbursementStat = useReimbursementCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: reimbursementStat?.data,
      isLoading: reimbursementStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`sick-leaves`} passHref legacyBehavior>
                  <Button
